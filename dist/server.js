...
const upload = multer({
    storage: multerS3({
        ...
        key: function (req, file, cb, err) {
            cb(err, Date.now().toString()); //업로드시 파일명 변경가능 겹치면 안되기떄문에 현재시간으로 업로드되게함
        }
    })
});
app.use(methodOverride('_method')); //여기까지 methodOverride
app.use(express_1.default.static(__dirname + '/public')); //public폴더참조를 위한 세팅
app.set('view engine', 'ejs'); //ejs 세팅 html에다가 db데이터 꽂아주는 html 템플릿. 근데 확장자가 ejs임 views 폴더안에 넣는게 국룰
app.use(express_1.default.json()); //res.body 쓰려면 이거랑,    body-parser에서 온것들
app.use(express_1.default.urlencoded({ extended: true })); //이거 필요
const session = require('express-session'); //로그인 form 만들어주는 라이브러리
const passport_1 = __importDefault(require("passport")); //회원등록기능 라이브러리
...
app.use(passport_1.default.initialize()); //app.use끼리 순서 중요..
app.use(session({
...
app.get('/write', ask_login, (req, res) => {
    if (req.session.passport === undefined)
        res.render('write.ejs', { singed: 0 });
    else if (req.session.passport)
        res.render('write.ejs', { signed: 1, user: req.user ? req.user : 0 });
});
...
app.post('/write/add', ask_login, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try { //try 안에 코드 실행해보고
...
app.post('/write/upload', upload.single('file'), (req, res) => {
    if (req.file) {
...
app.get('/list', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        let result = yield db.collection('post')
...
app.get('/list/:page', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
...
app.get('/searchWord/:page', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        let searchCon = [
            { $search: {
                    index: 'index_title',
        ...
        if (rowCount == 0) {
            ...
            if (req.session.passport === undefined) {
                ...
                res.render('searchResult.ejs', { 글목록: result, 글수: rowCount, signed: 1, keyword: req.query.keyword, user: req.user ? req.user : 0 });
        }
    }
    catch (e) {
        console.log(e);
...
app.get('/detail/:id', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        let result = yield db.collection('post').findOne({ _id: new mongodb_1.ObjectId(req.params.id) });
        let comResult = yield db.collection('comment').find({ parentId: new mongodb_1.ObjectId(req.params.id) }).toArray();
        if (result == null) {
...
app.get('/getContent/:id', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    let result = yield db.collection('post').findOne({ _id: new mongodb_1.ObjectId(req.params.id) });
    res.send({ result: result });
}));
app.post('/comment', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    yield db.collection('comment').insertOne({
        user: new mongodb_1.ObjectId(req.user._id),
        username: req.user.username,
        content: req.body.content,
        parentId: new mongodb_1.ObjectId(req.body.parentId)
    });
    res.redirect('back');
}));
app.get('/edit/:id', ask_login, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    let result = yield db.collection('post').findOne({ _id: new mongodb_1.ObjectId(req.params.id) });
    if (result != null) {
        let contentHTML = JSON.stringify(result.content).replace(/\n/, "<br>");
        console.log(contentHTML);
        if (req.session.passport === undefined)
            res.render('edit.ejs', { data: result, content: contentHTML, signed: 0 });
        else if (req.session.passport)
            res.render('edit.ejs', { data: result, content: contentHTML, signed: 1, user: req.user ? req.user : 0 });
    }
}));
app.put('/edit', ask_login, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try { //try 안에 코드 실행해보고
        yield db.collection('post').updateOne({ _id: new mongodb_1.ObjectId(req.body._id) }, {
...
//토스터ui editor에서 내용입력할때 이미지 업로드시(AWS에 업로드 먼저) 이미지 미리보기 표시용 post api
app.post('/edit/upload', upload.single('file'), (req, res) => {
    if (req.file) {
        res.json({ url: req.file.location });
    }
    else {
...
app.delete('/delete', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    yield db.collection('post').deleteOne({
        _id: new mongodb_1.ObjectId(req.query.docId),
        user: new mongodb_1.ObjectId(req.user._id)
    });
...
app.delete('/deleteOnDetail/post/', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    console.log(req.body._id);
    yield db.collection('post').deleteOne({ _id: new mongodb_1.ObjectId(req.body._id) });
    yield db.collection('comment').deleteMany({ parentId: new mongodb_1.ObjectId(req.body._id) });
    res.redirect('/list/1');
}));
//상세페이지에서 댓글 삭제시 delete 요청
app.delete('/deleteOnDetail/comment/', ask_login, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    yield db.collection('comment').deleteOne({ _id: new mongodb_1.ObjectId(req.query.comId) });
    res.json();
}));
...
passport_1.default.use(new LocalStrategy((inputUsername, inputPassword, cb) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        let result = yield db.collection('user').findOne({ username: inputUsername });
        if (!result) { //cb(1, 2, 3) 2에 false를 넣으면 로그인에 실패했다고 알려주는거
            return cb(null, false, { message: '가입되지 않은 아이디입니다 가입페이지로 이동합니다' });
        }
        if (yield bcrypt.compare(inputPassword, result.password)) { //bcrypt 기능으로 db에 저장된 비번과 유저가 입력한 비번과 비교
...
passport_1.default.serializeUser((user, done) => {
    process.nextTick(() => {
        done(null, { id: new mongodb_1.ObjectId(user._id), username: user.username }); //아래 req.logIn() 할때마다 세션 document에 기록되며 쿠키도 자동으로 만들어줌
    });
});
passport_1.default.deserializeUser((user, done) => __awaiter(void 0, void 0, void 0, function* () {
    let result = yield db.collection('user').findOne({ _id: new mongodb_1.ObjectId(user.id) }); //최신 유저정보를 갱신하기위해 db에서 추가적으로 id 조회 하지만 이렇게되면 비번까지 가져오게됨
    if (result !== null) {
...
//---------------------------------------------------------------------------------------
app.get('/login', ask_logout, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    if (req.session.passport === undefined)
        res.render('login.ejs', { signed: 0 });
    else if (req.session.passport)
        res.render('login.ejs', { signed: 1 });
}));
app.post('/login', (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    passport_1.default.authenticate('local', (error, user, info) => {
        if (error)
            return res.status(500).json(error); //로그인과정 오류시
...
// })
app.get('/account', ask_login, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    console.log(req.user);
    if (req.session.passport === undefined)
        res.render('account.ejs', { data: req.user, signed: 0 });
    else if (req.session.passport)
        res.render('account.ejs', { data: req.user, signed: 1, user: req.user ? req.user : 0 });
}));
app.get('/register', ask_logout, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    if (req.session.passport === undefined)
        res.render('register.ejs', { signed: 0 });
    else if (req.session.passport)
        res.render('register.ejs', { signed: 1 });
}));
app.post('/register', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    let hashed = yield bcrypt.hash(req.body.password, 10); //hashing 기능 ('문자', 몇번꼬아줄지)
    if (yield db.collection('user').findOne({ username: req.body.username })) {
        ...
        yield db.collection('user').insertOne({
...
//채팅방 추가
app.get('/addChatRoom', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    if (req.session.passport) {
        yield db.collection('chats').insertOne({
            member: { invitor: new mongodb_1.ObjectId(req.user._id), invitor_username: req.user.username, invitee: new mongodb_1.ObjectId(req.query.userId), invitee_username: req.query.username },
            date: new Date()
        });
...
//채팅방 입장
app.get('/chat/detail/:id', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    if (req.session.passport === undefined)
        res.render('chatDetail.ejs', { signed: 0 });
    else if (req.session.passport) {
        let result = yield db.collection('chats').findOne({ _id: new mongodb_1.ObjectId(req.params.id) });
        console.log(req.params.id);
        console.log(result);
        let chatResult = yield db.collection('chatMessages').find({ parentId: new mongodb_1.ObjectId(req.params.id) }).sort({ date: 1 }).toArray();
        res.render('chatDetail.ejs', { result: result, chatLogs: chatResult, user: req.user ? req.user : 0, signed: 1 });
    }
}));
//채팅방 리스트 게시판
app.get('/chat/list', ask_login, (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    let result = yield db.collection('chats').find({ $or: [{ "member.invitor": new mongodb_1.ObjectId(req.user._id) }, { "member.invitee": new mongodb_1.ObjectId(req.user._id) }] }).sort({ date: -1 }).toArray();
    let lastChat = new Array;
    for (let i = 0; i < result.length; i++) {
        lastChat.push(yield db.collection('chatMessages').find({ parentId: new mongodb_1.ObjectId(result[i]._id) }, { limit: 1 }).sort({ $natural: -1 }).toArray());
    }
...
//채팅방 삭제
app.delete('/deleteChat', (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    yield db.collection('chats').deleteOne({
        _id: new mongodb_1.ObjectId(req.query.docId)
    });
    yield db.collection('chatMessages').deleteMany({
        parentId: new mongodb_1.ObjectId(req.query.docId)
    });
    //console.log(req.query.docId)               //Query String으로 작성해서 보내온 데이터이기때문에 req.body나 req.params가 아닌 req.query로 꺼내쓴다
    //res.redirect('list')                      //ajax는 새로고침하지않는 장점인데 이따구로 navigate하면 안됨 어차피 적용 안되거든..
    res.json(); //이거 안쓰면 ajax 삭제 바로적용 안됨
...
io.on('connection', (socket) => {
    ...
    socket.on('message-send', (data) => __awaiter(void 0, void 0, void 0, function* () {
        yield db.collection('chatMessages').insertOne({
            text: data.msg,
            parentId: new mongodb_1.ObjectId(data.room),
            poster: new mongodb_1.ObjectId(data.poster),
